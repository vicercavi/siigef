...
$(document).on('ready', function() {
    ...
    var paginacion = function(pagina, nombrelista, datos) {
        $.post(_root_ + 'calidaddeagua/entidad/_paginacion_' + nombrelista + '/' + datos,
        {
            pagina: pagina,
            palabra:$("#palabra").val()
    ...
    $("body").on('click', '.estado-entidad', function() {
        cambiar_estado($(this).attr("identidad"),$(this).attr("estado"));
    });
    $("body").on('click', '.eliminar-entidad', function() {
        eliminar($(this).attr("identidad"));
    });
});

function buscar(palabra) {
    $.post(_root_ + 'calidaddeagua/entidad/_buscarEntidad',
            {
                palabra: palabra
            }, function(data) {
        $("#listaregistros").html('');
...
function cambiar_estado(id,estado) {
        ...
        if (_post && _post.readyState != 4) {
         ...
         _post = $.post(_root_ + 'calidaddeagua/entidad/_cambiarEstadoEntidad',
                {
...
function eliminar(id) {
        ...
        _post = $.post(_root_ + 'calidaddeagua/entidad/_eliminarEntidad',
                {
                    pagina: $(".pagination .active span").html(),
                    identidad: id,
                    palabra: $("#palabra").val(),
