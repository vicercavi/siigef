...
class registrarModel extends Model
{
    ...
    public function getFichaEstandar($condicion = "")
    {
        $post = $this->_db->query("SELECT * FROM ficha_estandar fie
...
WHERE fie.Esr_IdEstandarRecurso = $condicion ");				
    ...
    {
        $post = $this->_db->query("SELECT * FROM estandar_recurso esr
WHERE esr.Esr_IdEstandarRecurso = $condicion ");				
        return $post->fetchAll();
    }
	
	public function getRecurso($condicion = "")
    {
        $post = $this->_db->query("SELECT rec.Esr_IdEstandarRecurso FROM recurso rec
WHERE rec.Rec_IdRecurso = $condicion ");				
        return $post->fetch();
    }
...
WHERE Pai_Nombre = '$Pai_Nombre' ");				
	...
	//Para matriz Legal
	public function getNivelLegal($condicion = "")
    {
        $post = $this->_db->query("SELECT Nil_IdNivelLegal FROM nivel_legal
WHERE Nil_Nombre = '$condicion' ");				
        return $post->fetch();
    }
	
	public function getSubNivelLegal($idnivellegal,$subnivellegal)
    {
        $post = $this->_db->query("SELECT * FROM sub_nivel_legal snl
WHERE snl.Nil_IdNivelLegal = $idnivellegal AND snl.Snl_Nombre = '$subnivellegal'");				
        return $post->fetch();
    }
	
	public function getTemaLegal($idsubnivellegal,$temalegal)
    {
        $post = $this->_db->query("SELECT * FROM tema_legal tel
WHERE tel.Snl_IdSubNivelLegal = $idsubnivellegal AND tel.Tel_Nombre = '$temalegal'");				
        return $post->fetch();
    }
	
	public function registrarNivelLegal($Nil_Nombre)
    {
        $this->_db->prepare(
                "insert into nivel_legal (Nil_Nombre) values " .
                "(:Nil_Nombre)"
                )
                ->execute(array(
	...
	public function registrarSubNivelLegal($Snl_Nombre, $Nil_IdNivelLegal)
    {
        $this->_db->prepare(
                "insert into sub_nivel_legal (Snl_Nombre,Nil_IdNivelLegal) values " .
                "(:Snl_Nombre, :Nil_IdNivelLegal)"
                )
                ->execute(array(
	...
	public function registrarTemaLegal($Tel_Nombre, $Snl_IdSubNivelLegal)
    {
        $this->_db->prepare(
                "insert into tema_legal (Tel_Nombre,Snl_IdSubNivelLegal) values " .
                "(:Tel_Nombre, :Snl_IdSubNivelLegal)"
                )
                ->execute(array(
	...
	{
		$id = $this->_db->query(
                "SELECT * FROM matriz_legal WHERE Mal_FechaPublicacion = '$fechapublicacion' AND Mal_Entidad = '$entidad' AND Mal_NumeroNormas = '$numeronormas' AND Mal_Titulo = '$titulo' AND Mal_ArticuloAplicable = '$articuloaplicable' AND Mal_ResumenLegislacion = '$resumenlegislacion' AND Mal_FechaRevision = '$fecharevision' AND Mal_NormasComplemaentarias = '$normascomplementarias' AND Mal_TipoLegislacion = '$tipolegislacion' AND tel_idtemalegal = $tema AND pai_idpais = $pais AND Idi_IdIdioma = '$idioma' AND rec_idrecurso = $recurso AND Mal_PalabraClave = '$palabraclave'");
		if($id->fetch()){
			return true;
		}
		return false;
	...
	public function registrarLegislacion($Mal_FechaPublicacion, $Mal_Entidad, $Mal_NumeroNormas, $Mal_Titulo, $Mal_ArticuloAplicable, $Mal_ResumenLegislacion, $Mal_FechaRevision, $Mal_NormasComplemaentarias,$Mal_TipoLegislacion, $Tel_IdTemaLegal, $Pai_IdPais,$Rec_IdRecurso,$Mal_PalabraClave,$Idi_IdIdioma)
    {
    	
		
        $this->_db->prepare(
                "insert into matriz_legal (Mal_FechaPublicacion,Mal_Entidad,Mal_NumeroNormas,Mal_Titulo,Mal_ArticuloAplicable,Mal_ResumenLegislacion,Mal_FechaRevision,Mal_NormasComplemaentarias,Mal_TipoLegislacion,Tel_IdTemaLegal,Pai_IdPais,Rec_IdRecurso,Mal_Estado,Mal_PalabraClave,Idi_IdIdioma) values " .
                "(:Mal_FechaPublicacion, :Mal_Entidad, :Mal_NumeroNormas, :Mal_Titulo, :Mal_ArticuloAplicable, :Mal_ResumenLegislacion, :Mal_FechaRevision, :Mal_NormasComplemaentarias, :Mal_TipoLegislacion, :Tel_IdTemaLegal, :Pai_IdPais, :Rec_IdRecurso,1,:Mal_PalabraClave,:Idi_IdIdioma)"
                )
                ->execute(array(
                    ...
                    ':Mal_TipoLegislacion' => $Mal_TipoLegislacion,
...
WHERE var.Var_Nombre = '$variable' ");				
	...
	public function registrarVariables($Var_Nombre,$Var_Abreviatura,$Var_Medida)
    {
        $this->_db->prepare(
                "insert into variables_estudio (Var_Nombre,Var_Abreviatura,Var_Medida) values " .
                "(:Var_Nombre, :Var_Abreviatura, :Var_Medida)"
                )
                ->execute(array(
...
WHERE autor.Aut_Nombre = '$autor' ");				
	...
	public function getTemaDublin($temadublin = "")
    {
        $post = $this->_db->query("SELECT * FROM tema_dublin ted
WHERE ted.Ted_Descripcion = '$temadublin' ");				
        return $post->fetch();
    }
	
	public function getTipoDublin($tipodublin = "")
    {
        $post = $this->_db->query("SELECT * FROM tipo_dublin tid
WHERE tid.Tid_Descripcion = '$tipodublin' ");				
        return $post->fetch();
    }
...
WHERE dub.Dub_Titulo = '$titulo' AND dub.Tid_IdTipoDublin = $tid_dublin ");				
	...
	public function registrarTemaDublin($Ted_Descripcion)
    {
        $this->_db->prepare(
                "insert into tema_dublin (Ted_Descripcion) values " .
                "(:Ted_Descripcion)"
                )
                ->execute(array(
	...
	public function registrarTipoDublin($Tid_Descripcion)
    {
        $this->_db->prepare(
                "insert into tipo_dublin (Tid_Descripcion) values " .
                "(:Tid_Descripcion)"
                )
                ->execute(array(
