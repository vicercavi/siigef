...
function construirpuntos(datos, variables) {
    ...
       $.each(datos.estacion, function(posicion, punto) {
        ...
        $.each(punto.params, function(posicion2, param) {
            ...
            valorparam = valorparam + '<tr><td>' + param.nombreParametro + " " + abrevi + '</td><td>' + param.unidadMedida + '</td>' + eca + '<td>' + param.fecha + '</td><td><a href="' + _root_ + 'monitoreo/variable/' + param.ParametroId + '" target="_blank" data-toggle="tooltip" data-placement="bottom" class="link-home" title="Listar todas las estaciones donde se estudió la variable" >ver</a></td></tr>';
        });
        ...
        var contentString = '<div  class="panel panel-default" style="width: 410px;">' +
                ...
                '<a data-toggle="tooltip" data-placement="bottom" class="link-home" title="Listar todas las viariables estudiadas" href="' + _root_ + 'monitoreo/estacion/' + punto.EstacionId + '" target="_blank"> Estación ' + punto.nombrePunto + '</a></h4>' +
                '<div class="div4">' +
                ...
                valorparam +
                '</div></div>' +
                '<div class="row-fluid"><div style="display: inline-block;margin: 5px 10px 0 0;"><a href="' + _root_ + "monitoreo/_exportaEstacionMonitoreo/" + punto.EstacionId + "/" + idvars + '" target="_blank"><i class="icon-share"></i>Exportar</a></div></div>' +
                '</div>';
